...
describe("fetch Todos", () => {
  beforeAll(async () => {
    ...
    for (let i = 0; i < 5; i++) {
      const todoRes = await request(app)
        .post(`${fetchTodosAPI}/create-todo`)
        .set("Cookie", cookies)
        .send({
          title: `Task ${i + 1}`,
          status: "todo",
  ...
  it("should fetch todos by search keyword in title or description", async () => {
    await request(app).post(createTodoAPI).set("Cookie", cookies).send({
...
describe("delete Todos", () => {
  ...
  it("should return 401 if not authenticated", async () => {
    const res = await request(app).post(deleteTodosAPI).send({ todoIds });
