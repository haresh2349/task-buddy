...
export const SnackBar = ({type,message,showTime=5000}:SnackbarProps) => {
    ...
    return <div className={`rounded absolute right-10 bottom-10 p-4 flex justify-between items-center gap-10 ${bgColor} ${borderColor}`}>
    ...
    <span className="cursor-pointer" onClick={() => dispatch(activateSnackbar({ message: null, type: null }))}>
      <svg className={`fill-current h-6 w-6 ${iconColor}`} role="button" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
        <title>Close</title>
