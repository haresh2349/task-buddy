...
export const Login = ({setShowSignup}:{setShowSignup:React.Dispatch<React.SetStateAction<boolean>>}) => {
    const dispatch = useAppDispatch();
    const Navigate = useNavigate();
    const [isLoading,setIsLoading] = useState<boolean>(false)
    const [errors,setErrors] = useState<LoginFormData>({email:"",password:""})
    ...
    return <div className="w-1/2 h-1/2 flex items-center justify-center min-h-screen">
      <div className="w-full max-w-md p-8 space-y-8 bg-white rounded-lg shadow-md">
        ...
        <form className="mt-8 space-y-8" onSubmit={(e) => handleSubmitLogin({e,formData,setErrors,dispatch,Navigate,setIsLoading})}>
          <div className="rounded-md space-y-4">
            ...
            <div>
                <p className="text-sm font-medium text-gray-700">Don't have an account? <span onClick={() => setShowSignup(true)} className="underline cursor-pointer">Create new account</span> </p>
            </div>
          </div>
          <div >
            <button
