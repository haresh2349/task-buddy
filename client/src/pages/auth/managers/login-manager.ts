...
import { HandleLoginProps, HandleLoginUserReponse, HandleSubmitLoginProps, LoginFormData } from "../../../types/auth-types";

export const handleSubmitLogin = async ({e,formData,setErrors,dispatch,Navigate,setIsLoading}:HandleSubmitLoginProps) => {
    e.preventDefault()
    ...
    const loginResponse = await handleLoginUser({payload:{email,password},dispatch,setIsLoading})
    if(loginResponse?.result?.accessToken){
        dispatch(activateSnackbar({message:loginResponse?.message,type:"success",}))
        setTimeout(() => {
...
const handleLoginUser = async ({payload,dispatch,setIsLoading}:HandleLoginProps) => {
    try {
        setIsLoading(true);
        const response = await api.post<HandleLoginUserReponse>(API_END_POINTS.loginUser,payload,{"x-auth-require":false});
