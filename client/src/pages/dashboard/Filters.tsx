...
import { clearTodosToDelete, setTodosToDelete } from "../../store/slices/todos-slice";
    ...
    <div className="w-full px-4 py-2 flex justify-between items-center">
        ...
        <div className="w-full md:w-auto justify-between flex gap-4">
            <div className="flex gap-2 items-center border rounded-lg p-2">
                ...
                <input onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search" className="border-bottom"/>
            </div>
            {todosToDelete?.length > 0 && <button onClick={() => setShowDeleteDailog(true)} className="flex items-center gap-2 font-medium bg-red-500 px-4 py-1 rounded text-white"><MdDelete/> Delete</button>}
            <button onClick={() => setShowTodoModal(true)} className={`${todosToDelete?.length > 0 ? "hidden" :""} bg-[#7B1984] font-medium px-4 py-1 rounded text-white`}>Add Task</button>
        </div>
    </div>
