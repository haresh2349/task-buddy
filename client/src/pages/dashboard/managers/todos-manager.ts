...
import {
  ...
  EditTodo,
  GetTodosResponse,
  HandleGetSingleTodosProps,
  SingleTodoAPIReponse,
  Todo,
...
export const handleGetSingleTodo = async ({
  id,
  setIsLoading,
  dispatch,
  setTaskDetails,
  ...
  try {
    ...
    const response = await api.get<SingleTodoAPIReponse>(
      `${API_END_POINTS.getTodos}/${id}`
    );
    const data = response?.data?.result;
    setTaskDetails(data);
  } catch (error) {
...
export const handleSubmitCreateTask = async ({
  e,
  setErrors,
  formData,
  dispatch,
...
export const createTodo = async ({
  ...
  dispatch,
  setIsLoading,
}: CreateTodoProps): Promise<CreateTodoAPIResponse | void> => {
  try {
    ...
    handleApiFailure({
      error,
      defaultMessage: "Failed to create a task!",
      dispatch,
    });
  } finally {
    setIsLoading(false);
...
export const handlegetGroupedTodos = (
  todos: Todo[]
): { todo: Todo[]; inprogress: Todo[]; done: Todo[] } => {
  let todo = [] as Todo[];
...
export const handleSubmitEditTask = async ({
  e,
  taskId,
  setErrors,
  taskDetails,
...
export const handleUpdateTask = async ({
  ...
  dispatch,
  setIsLoading,
}: HandleUpdateTaskProps): Promise<HandleUpdateTaskAPIResponse | void> => {
  try {
    setIsLoading && setIsLoading(true);
    const response = await api.patch<HandleUpdateTaskAPIResponse>(
      `${API_END_POINTS.updateTodos}/${id}`,
      payload
    );
    const data = response?.data;
    return data;
...
export const handleDeleteTask = async ({
  ...
  try {
    setIsLoading(true);
    const response = await api.post(API_END_POINTS.deleteTodos, {
      todoIds: ids,
    });
